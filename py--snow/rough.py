...
conn = snowflake.connector.connect(
    user='data_loader',
    password='Manikanta@818708',
    account='ueqnjab-gxb10120',
    warehouse='azware',
    database='AZDATABASE',
    application = 'first_raw_load'
)

print( "2.1 object =>" ,type(conn)) 
print(conn.user)
...
try :
    ...
    print("2.3 object =>", type(one_row))
    print("2.4 object =>", one_row[0])
    print(one_row[2])
finally:
    
    print("2.5 object =>", type(conn))
...
curs.execute("create or replace schema raw")
def generate_create_table_sql(table_name, df):
    ...
    create_table_sql = f"""
    create or replace table ADVWORKS.{table_name}(
    {column_names})
    """
...
for url in csv_urls:
    ...
    try:
        df = pd.read_csv(url, delimiter=",", encoding="latin1")  #pandas uses utf-8 by default, but some CSVs may require latin1
        print(f"\n file: {table_name}")
        print("Columns:" ,df.columns.tolist())
        print("Types:", df.dtypes)
